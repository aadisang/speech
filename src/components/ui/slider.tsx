...
export function Slider<Value extends number | readonly number[] = number>({
  ...
  max = 100,
  ...props
}: SliderPrimitive.Root.Props<Value>): React.ReactElement {
        ...
        >
          ...
          {Array.from({ length: values.length }, (_, index) => (
            <SliderPrimitive.Thumb
              className="block size-5 shrink-0 select-none rounded-full border border-input bg-white not-dark:bg-clip-padding shadow-xs/5 outline-none transition-[box-shadow,scale] before:absolute before:inset-0 before:rounded-full before:shadow-[0_1px_--theme(--color-black/4%)] has-focus-visible:ring-[3px] has-focus-visible:ring-ring data-dragging:scale-120 motion-reduce:transition-none motion-reduce:data-dragging:scale-100 sm:size-4 dark:border-background [:has(*:focus-visible),[data-dragging]]:shadow-none"
